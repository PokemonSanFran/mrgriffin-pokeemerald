...
enum MugshotImplementation
{
    MUGSHOT_NONE,
    MUGSHOT_SHINY_DRAGON_HUNTER,
};
...
struct Parsed
{
    ...
    enum MugshotImplementation mugshot_implementation;
};
...
static bool show_parse_error(struct Parser *p)
{
    // Print error message.
    int n;
    n = fprintf(stderr, "%s:%d: ", p->source->path, p->error_location.line);
...
static bool parse_pragma(struct Parser *p, struct Parsed *parsed)
{
    ...
    else if (is_literal_token(&id, "ivs"))
    {
        ...
        if (!token_stats(p, &t, &parsed->default_ivs, true))
            return show_parse_error(p);
    }
    else if (is_literal_token(&id, "mugshot"))
    {
        if (parsed->mugshot_implementation != MUGSHOT_NONE)
            return set_show_parse_error(p, p_.location, "duplicate #pragma trainerproc mugshot");
        skip_whitespace(&p_);
        if (!match_identifier(&p_, &id))
        {
            return set_show_parse_error(p, p_.location, "expected mugshot implementation");
        }
        else if (is_literal_token(&id, "ShinyDragonHunter"))
        {
            parsed->mugshot_implementation = MUGSHOT_SHINY_DRAGON_HUNTER;
        }
        else
        {
            return set_show_parse_error(p, p_.location, "unknown mugshot implementation");
        }
    }
    else
    {
...
static bool parse_trainer(struct Parser *p, const struct Parsed *parsed, struct Trainer *trainer)
{
    ...
    *trainer = (struct Trainer) {};
    ...
    while (parse_attribute(p, &key, &value))
    {
        ...
        {
            if (parsed->mugshot_implementation == MUGSHOT_NONE)
                any_error = !set_show_parse_error(p, key.location, "'Mugshot' without #pragma trainerproc mugshot <implementation>");
    ...
    for (int i = 0; i < PARTY_SIZE; i++)
    {
        ...
        while (parse_attribute(p, &key, &value))
        {
            ...
            else if (is_literal_token(&key, "IVs"))
            {
                ...
                if (!token_stats(p, &value, &pokemon->ivs, parsed->default_ivs_line == 0))
                    any_error = !show_parse_error(p);
            }
        ...
        if (!pokemon->ivs_line && !parsed->default_ivs_line)
...
static void fprint_trainers(const char *output_path, FILE *f, struct Parsed *parsed)
{
    ...
    for (int i = 0; i < parsed->trainers_n; i++)
    {
        ...
        switch (parsed->mugshot_implementation)
        {
        case MUGSHOT_NONE:
            break;

        case MUGSHOT_SHINY_DRAGON_HUNTER:
        ...
        for (int j = 0; j < trainer->pokemon_n; j++)
        {
            ...
            {
                fprintf(f, "#line %d\n", pokemon->nature_line);
                fprintf(f, "            .nature = TRAINER_PARTY_NATURE(");
                fprint_constant(f, "NATURE", pokemon->nature);
                fprintf(f, "),\n");
            }
            else
            {
                fprintf(f, "            .nature = TRAINER_PARTY_NATURE(NATURE_HARDY),\n");
            }
...
int main(int argc, char *argv[])
{
    ...
    struct Parsed parsed = {
        ...
        .mugshot_implementation = MUGSHOT_NONE,
    };
